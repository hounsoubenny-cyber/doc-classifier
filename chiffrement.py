...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 20 18:05:31 2026
...
class FernetManager:
    def __init__(self, password:str, salt:bytes=None):
        ...
        # print('Voici votre salt, veuillez à NE JAMAIS LE PERDRE !', self.salt)
    ...
    def _gen_key(self, password, salt):
        if not password:
            raise ValueError('Mot de passe vide !')
        p = password.encode() if isinstance(password, str) else password
        key = bcrypt.kdf(p, salt=salt, desired_key_bytes=32, rounds=ROUNDS)
